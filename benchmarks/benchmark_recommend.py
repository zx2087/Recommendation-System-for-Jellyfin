...
Environment variables:
    ...
    NUM_CANDIDATES    Candidate movies per req (default: 20)
"""
...
NUM_CANDIDATES = int(os.getenv("NUM_CANDIDATES", "20"))

EMBEDDING_DIM = 384

# Pre-generate a fixed payload (same for every request — isolates server-side latency)
_rng = np.random.default_rng(42)
_user_emb: list[float] = _rng.standard_normal(EMBEDDING_DIM).astype(np.float32).tolist()
_candidate_movies = [
    {
        "movie_id": f"movie_{i:04d}",
        "movie_embedding": _rng.standard_normal(EMBEDDING_DIM).astype(np.float32).tolist(),
    }
    for i in range(NUM_CANDIDATES)
]

BASE_PAYLOAD: dict = {
    "request_id": "bench_req_0000",
    "user_id": "user_bench",
    "timestamp": "2024-01-01T12:00:00Z",
    "request_k": 10,
    "user_embedding": _user_emb,
    "candidate_movies": _candidate_movies,
    "client_context": {"surface": "benchmark", "device_type": "server", "app_version": "bench"},
}


def send_one_request(session: requests.Session, request_index: int) -> dict:
    payload = {**BASE_PAYLOAD, "request_id": f"bench_req_{request_index:06d}"}
...
def warmup(session: requests.Session, n: int = 5) -> None:
    for i in range(n):
        try:
            payload = {**BASE_PAYLOAD, "request_id": f"warmup_{i}"}
            session.post(ENDPOINT, json=payload, timeout=TIMEOUT_SECONDS)
        except Exception:
            pass
